...
public sealed class DNCAlt(RotationModuleManager manager, Actor player) : Attackxan<AID, TraitID>(manager, player)
{
    ...
    {
        var def = new RotationModuleDefinition("xan-erica DNC", "Dancer", "Optimized rotation (xan-erica)|Ranged", "xan, erica", RotationModuleQuality.Good, BitMask.Build(Class.DNC), 100, 100);
    ...
    public override void Exec(StrategyValues strategy, Enemy? primaryTarget)
    {
        ...
        if (IsDancing)
        {
            ...
            if (ShouldFinishDance(TechStepLeft, 0))
            {
                ...
                {
                    PushOGCD(AID.Devilment, Player, 100, 0.050f);
                    PushOGCD(AID.Devilment, Player, 100, 0.150f);
                    PushOGCD(AID.Devilment, Player, 100, 0.250f);
                    PushOGCD(AID.Devilment, Player, 100, 0.350f);
                    PushOGCD(AID.Devilment, Player, 100, 0.450f);
                    PushOGCD(AID.Devilment, Player, 100, 0.550f);
                }
            }
        ...
        if (DanceOfTheDawnLeft > GCD && DanceOfTheDawnLeft <= GCDLength && Esprit >= 50)
        ...
        if (ReadyIn(AID.FinishingMove) < GCDLength && FinishingMoveLeft >= GCDLength && NumDanceTargets > 0)
            PushGCD(AID.FinishingMove, Player, 5);

        if (DanceOfTheDawnLeft > GCD && Esprit >= 50)
            PushGCD(AID.DanceOfTheDawn, BestRangedAOETarget);

        if (DanceOfTheDawnLeft > GCD + GCDLength && FlourishingFinishLeft > GCD && Esprit < 25)
        ...
        if (LastDanceLeft > 0 && (ReadyIn(AID.StandardStep) > GCD && ReadyIn(AID.StandardStep) <= GCD + GCDLength || ReadyIn(AID.FinishingMove) > GCD && ReadyIn(AID.FinishingMove) <= GCD + GCDLength))
        ...
        if (0 < FlourishingFinishLeft && FlourishingFinishLeft < GCDLength && OnCooldown(AID.Devilment) && NumDanceTargets > 0)
            PushGCD(AID.Tillana, Player);

        //If we have technical finish for starting+finishing dance (2 GCDs) + 2 steps, do standard step ASAP
        if (TechFinishLeft > 2 * GCDLength + 2 && shouldStdStep)
        {
            PushGCD(AID.StandardStep, Player, 3);
        }

        // TODO combine this with above
        if (LastDanceLeft > GCDLength && TechFinishLeft > GCDLength && OnCooldown(AID.Devilment))
        ...
        if (LastDanceLeft > 0 && LastDanceLeft < 3 * GCDLength + ReadyIn(AID.TechnicalStep) + 4)
            PushGCD(AID.LastDance, BestRangedAOETarget);
        ...
        if (FlourishingFinishLeft > GCDLength && OnCooldown(AID.Devilment) && NumDanceTargets > 0 && Esprit < 50)
            PushGCD(AID.Tillana, Player);
    ...
    {
        return (!OnCooldown(AID.StandardStep) || ReadyIn(AID.StandardStep) <= GCDLength) && NumDanceTargets > 0;
    }
